...
function parentMaps(links: ParentChildRelationship[]) {
  ...
  for (const l of links) {
    if (l.parent_role === "father") {
      fatherOf.set(l.child_member_id, l.parent_member_id);
    } else {
      motherOf.set(l.child_member_id, l.parent_member_id);
    }
  }
...
function sharedChildren(
  ...
  // نفحص جميع الأطفال الموجودين في أي من الخريطتين
  const allChildren = new Set<string>([
    ...Array.from(fatherOf.keys()),
    ...Array.from(motherOf.keys()),
  ]);
...
function computeGenerations(
  ...
  // نوحّد الجيل بين الزوجين، ثم نضمن أن الأبناء أسفل الوالدين
  for (let i = 0; i < 32; i++) {
    let changed = false;
...
export function buildFlowGraph(input: TreeGraphInput): { nodes: Node[]; edges: Edge[] } {
  ...
  /**
   * ملاحظة:
   * هذا التصميم ما يزال يفترض شريكًا واحدًا لكل عضو في التخطيط.
   * إذا أردت دعم تعدد الزيجات فعليًا، يجب إعادة تصميم هذا الجزء.
   */
  ...
  const rootIds = memberList
    .filter((m) => !fatherOf.has(m.id) && !motherOf.has(m.id))
    .map((m) => m.id);
  ...
  function measureUnit(u: Unit): number {
    const k = unitKey(u);
    const cached = measureCache.get(k);
    ...
    if (kids.length === 0) {
      ...
      for (let i = 0; i < childUnits.length; i++) {
        total += measureUnit(childUnits[i]!);
        if (i < childUnits.length - 1) total += SUBTREE_PAD;
      }

      result = Math.max(leafWidth(u), total);
    }

    measureCache.set(k, result);
    return result;
  }

  function placeUnit(u: Unit, centerX: number, fallbackDepth: number): void {
    const k = unitKey(u);

    // يمنع إعادة تموضع نفس الشخص/الزوجين أكثر من مرة
    if (placedUnits.has(k)) return;
    placedUnits.add(k);

    const rowLevel =
      u.kind === "pair"
        ? Math.max(genLevel.get(u.a) ?? fallbackDepth, genLevel.get(u.b) ?? fallbackDepth)
        : (genLevel.get(u.id) ?? fallbackDepth);

    const yRow = rowLevel * VERTICAL;

    if (u.kind === "pair") {
      centers.set(u.a, {
        cx: centerX - COUPLE_HALF_SPREAD,
        cy: yRow,
      });
    ...
    const childUnits = dedupeUnits(kids.map((id) => unitForMember(id, partnerOf, memberIds)));
    const widths = childUnits.map((cu) => measureUnit(cu));
    const totalW =
      widths.reduce((s, w) => s + w, 0) + SUBTREE_PAD * Math.max(0, childUnits.length - 1);
    ...
    for (let i = 0; i < childUnits.length; i++) {
      ...
      const w = widths[i]!;
      placeUnit(cu, cursor + w / 2, nextLevel);
      cursor += w + SUBTREE_PAD;
    }
  }

  const rootWidths = rootUnits.map((ru) => measureUnit(ru));
  ...
  // في حال وُجد أعضاء لم يتم تموضعهم لأي سبب، نضعهم في أماكن احتياطية
  const unplacedMembers = memberList.filter((m) => !centers.has(m.id));
  if (unplacedMembers.length > 0) {
    let extraX = rx;

    for (const m of unplacedMembers) {
      const y = (genLevel.get(m.id) ?? 0) * VERTICAL;
      centers.set(m.id, { cx: extraX, cy: y });
      extraX += NODE_W + ROOT_GAP;
    }
  }

  // توسيط الرسم أفقيًا حول الصفر
  const allCx = [
    ...memberList.map((m) => centers.get(m.id)?.cx).filter((x): x is number => x !== undefined),
    ...unionNodes.map((u) => u.cx),
  ];

  const avgX = allCx.length
    ? allCx.reduce((a, b) => a + b, 0) / allCx.length
    : 0;
  ...
  const nodes: Node[] = memberList.map((m) => {
    const c = centers.get(m.id) ?? {
      cx: 0,
      cy: (genLevel.get(m.id) ?? 0) * VERTICAL,
    };
